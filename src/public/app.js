document.addEventListener('DOMContentLoaded', () => {
  ...
  const copyAllCdnBtn = document.getElementById('copy-all-cdn-btn');
  ...
  // State Variables
  let selectedFiles = [];
  let githubConfig = { owner: '', repo: '', branch: 'main', path: 'assets' };
  ...
  /* ==========================================================================
     SETTINGS & CONFIG (SILENT BACKGROUND FETCH)
     ========================================================================== */
  async function loadConfig() {
    try {
      ...
      if (data.success && data.config) {
        githubConfig = data.config;
      }
    } catch (_err) {
      console.error('Không thể tải thông tin cấu hình từ máy chủ.');
    }
  }
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed
          ...
          .filter(
            (item) => item.length > 0 && (item.startsWith('http://') || item.startsWith('https://'))
          );
      }
    } catch (_e) {
    ...
    try {
      ...
      if (data.success && data.results) {
        ...
        if (failCount === 0) {
          ...
          showToast(
            `Tải lên hoàn tất. Thành công: ${successCount}, Thất bại: ${failCount}`,
            'warning'
          );
        }
          ...
          if (data.success && data.results) {
            ...
            setTimeout(() => {
              ...
              if (failCount === 0) {
                ...
                showToast(
                  `Đã tải lên xong. Thành công: ${successCount}, Thất bại: ${failCount}`,
                  'warning'
                );
              }
  ...
  function displayResults(results) {
    ...
    if (successfulResults.length > 1) {
      ...
      // Gán sự kiện cho Copy All CDN/Download Links
      copyAllCdnBtn.onclick = () => {
        const allDownloadLinks = successfulResults.map((r) => r.cdnUrl || r.downloadUrl).join('\n');
    ...
    results.forEach((res, idx) => {
      ...
      if (res.success) {
        ...
        const directLink = res.cdnUrl || res.downloadUrl;
        ...
        itemCard.innerHTML = `
          ...
          <div class="bulk-result-grid">
            ...
            <div class="bulk-result-links">
              ...
              <div class="form-group">
                <div class="copy-input-group">
                  <input type="text" class="download-link-input" readonly value="${directLink}">
